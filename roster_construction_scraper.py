"""
Roster Construction Scraper
=============================
Reads roster-construction screenshots posted in #nil-roster-team-stats
(one screenshot per conference, showing each team's current-roster NIL
spend broken out by position group), uses Gemini vision to extract
every row, keeps ONLY user-controlled teams (per roster.py), and
writes/updates roster_construction_<season>.csv.

Meant to be triggered MANUALLY (workflow_dispatch only, no schedule) --
a once-per-season upload, not something to poll for.

Same overlap/upsert reasoning as recruiting_scraper.py: screenshots
require scrolling, so some teams may appear in more than one image in a
single batch -- rows are upserted by (Season, Team), so this is safe.

This is a DIFFERENT pool of NIL than recruiting_ranks_<season>.csv --
that file tracks NIL committed to an in-progress incoming recruiting
class; this one tracks the active roster's total NIL spend by position,
a separate number at a separate point in time. Deliberately not
combined into a single "total program NIL" anywhere, since doing so
would imply they're the same kind of number when they aren't.

Season resolution: identical approach to recruiting_scraper.py --
--season CLI flag wins, otherwise defaults to (newest dynasty_data
season) + 1, announced clearly in the run's summary post rather than
silently guessed.

Required environment variables:
    DISCORD_TOKEN                   Bot token (same one everything else uses)
    GENAI_API_KEY                   Gemini API key
    ROSTER_CHANNEL_ID                #nil-roster-team-stats channel ID
...
Usage:
    python roster_construction_scraper.py --season 2027
    python roster_construction_scraper.py                  -> auto-defaults, see above
"""
import asyncio
import csv
import glob
...
REQUIRED = [("DISCORD_TOKEN", BOT_TOKEN), ("GENAI_API_KEY", GEMINI_API_KEY), ("ROSTER_CHANNEL_ID", ROSTER_CHANNEL_ID)]
...
STATE_FILE = "processed_roster_images.json"
FAILED_FILE = "failed_roster_images.json"
CSV_HEADER = [
    "Season", "Team", "Conference", "NIL_Total",
    "NIL_QB", "NIL_RB", "NIL_WR", "NIL_TE", "NIL_OL", "NIL_DL", "NIL_LB", "NIL_DB", "NIL_KP",
]

client = genai.Client(api_key=GEMINI_API_KEY)

VISION_PROMPT = """
Analyze this college football roster NIL-spend screenshot. It shows one
conference's teams with current-roster NIL spend broken out by position group.
...
- "teams": A list of objects, one per row visible in the table, each with:
    ...
    - "nil_total": The number in the NIL column (ignore the diamond icon), as an integer.
    - "qb": The number in the QB column, as an integer.
    - "rb": The number in the RB column, as an integer.
    - "wr": The number in the WR column, as an integer.
    - "te": The number in the TE column, as an integer.
    - "ol": The number in the OL column, as an integer.
    - "dl": The number in the DL column, as an integer.
    - "lb": The number in the LB column, as an integer.
    - "db": The number in the DB column, as an integer.
    - "kp": The number in the K/P column, as an integer.
...
async def parse_roster_image_with_vision(image_bytes: bytes, filename: str) -> dict | None:
...
def get_current_season(directory: str = ".") -> int | None:
    ...
    found, or None if there isn't one yet. Deliberately duplicated
    (rather than importing rta_logic.py or recruiting_scraper.py) to
    keep this scraper standalone, same reasoning as the rest of the
    scraper scripts not depending on each other."""
...
def resolve_season(directory: str = ".") -> tuple[int, bool]:
    ...
    otherwise defaults to get_current_season()+1 (roster construction,
    like recruiting, represents the UPCOMING season). Falls back to the
    current calendar year only if there's no dynasty_data file at all
    yet to infer from."""
...
def upsert_rows(season: int, new_rows: list[dict], directory: str = ".") -> int:
    """Merges new_rows into roster_construction_<season>.csv, keyed on
    (Season, Team) -- same reasoning as recruiting_scraper.py's
    upsert_rows: a team appearing in more than one screenshot this run
    (or reappearing on a rerun) just overwrites its own row."""
    path = os.path.join(directory, f"roster_construction_{season}.csv")
    existing = {}
...
async def on_ready():
    try:
        ...
        if channel is None:
            log.error("Could not find channel %s -- check ROSTER_CHANNEL_ID and bot permissions.", ROSTER_CHANNEL_ID)
            await bot.close()
            return
        ...
        for msg in messages:
            for attachment in msg.attachments:
                ...
                data = await parse_roster_image_with_vision(image_bytes, attachment.filename)
                if data is None:
                ...
                for team_row in data.get("teams", []):
                    ...
                    all_kept_rows.append({
                        ...
                        "NIL_Total": team_row.get("nil_total", ""),
                        "NIL_QB": team_row.get("qb", ""),
                        "NIL_RB": team_row.get("rb", ""),
                        "NIL_WR": team_row.get("wr", ""),
                        "NIL_TE": team_row.get("te", ""),
                        "NIL_OL": team_row.get("ol", ""),
                        "NIL_DL": team_row.get("dl", ""),
                        "NIL_LB": team_row.get("lb", ""),
                        "NIL_DB": team_row.get("db", ""),
                        "NIL_KP": team_row.get("kp", ""),
                    })
        ...
        log.exception("Unhandled error during roster construction scrape:")
        raise
    finally:
        await bot.close()
...
def main():
    ...
    if error_text:
        notify.post_alert(SUMMARY_CHANNEL_ID, BOT_TOKEN, f"❌ Roster Construction Scraper FAILED: {error_text}")
        notify.post_log_file(ADMIN_LOG_CHANNEL_ID, BOT_TOKEN, "roster_construction_scraper")
        sys.exit(1)
    ...
    if _run_result["path"]:
        alert = (
            f"✅ Roster Construction Scraper: season {season}{season_note} -- "
        ...
        alert = "✅ Roster Construction Scraper: ran, no new roster screenshots found."
    else:
        alert = f"⚠️ Roster Construction Scraper: processed {_run_result['processed']} image(s) but found no user-team rows to keep -- check ROSTER_CHANNEL_ID and roster.py."
